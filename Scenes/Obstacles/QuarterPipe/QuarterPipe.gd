...
func _ready() -> void:
	gen_arc_points($Centre.position, PI/2, PI)
...
func gen_arc_points(center: Vector2, start_angle: float, end_angle: float):
	...
	new_maxbounds.size = Vector2(radius*1.1, radius*1.1)
	$MaxBounds/CollisionShape2D.shape = new_maxbounds
	$MaxBounds/CollisionShape2D.position = Vector2(-radius/2, radius/2) + $Centre.position
